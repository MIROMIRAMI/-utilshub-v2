...
type BmrProps = {
    ...
    age: number; setAge: (value: number) => void;
    weight: number; setWeight: (value: number) => void;
    height: number; setHeight: (value: number) => void;
    bmr: number;
};

const BmrCalculator = ({ gender, setGender, age, setAge, weight, setWeight, height, setHeight, bmr }: BmrProps) => {
    return (
...
const CalorieCalculator = ({ bmr }: { bmr: number }) => {
    ...
    const activityFactors = [ { label: 'Sedentary (office job)', value: 1.2 }, { label: 'Light Exercise (1-2 days/wk)', value: 1.375 }, { label: 'Moderate Exercise (3-5 days/wk)', value: 1.55 }, { label: 'Heavy Exercise (6-7 days/wk)', value: 1.725 }, { label: 'Athlete (2x per day)', value: 1.9 }, ];
...
const BodyFatCalculator = ({ gender, height, neck, setNeck, waist, setWaist, hip, setHip }: { gender: string; height: number; neck: number; setNeck: (v: number) => void; waist: number; setWaist: (v: number) => void; hip: number; setHip: (v: number) => void; }) => {
    const bodyFatPercentage = useMemo(() => {
        if (!height || !neck || !waist || (gender === 'female' && !hip)) return 0;
        if (gender === 'male') { if ((waist - neck) <= 0) return 0; return 495 / (1.0324 - 0.19077 * Math.log10(waist - neck) + 0.15456 * Math.log10(height)) - 450; }
        else { if ((waist + hip - neck) <= 0) return 0; return 495 / (1.29579 - 0.35004 * Math.log10(waist + hip - neck) + 0.22100 * Math.log10(height)) - 450; }
...
const PaceCalculator = () => { const [distance, setDistance] = useState('10'); const [hours, setHours] = useState('0'); const [minutes, setMinutes] = useState('55'); const [seconds, setSeconds] = useState('0'); const [unit, setUnit] = useState('km'); const paceResult = useMemo(() => { const dist = parseFloat(distance); const h = parseInt(hours) || 0; const m = parseInt(minutes) || 0; const s = parseInt(seconds) || 0; if (!dist || dist <= 0) return { pace: '00:00', speed: '0.0' }; const totalTimeInSeconds = (h * 3600) + (m * 60) + s; if (totalTimeInSeconds === 0) return { pace: '00:00', speed: '0.0' }; const secondsPerUnit = totalTimeInSeconds / dist; const speed = dist / (totalTimeInSeconds / 3600); const paceMinutes = Math.floor(secondsPerUnit / 60); const paceSeconds = Math.round(secondsPerUnit % 60); return { pace: `${String(paceMinutes).padStart(2, '0')}:${String(paceSeconds).padStart(2, '0')}`, speed: speed.toFixed(2) }; }, [distance, hours, minutes, seconds, unit]); return ( <div className="space-y-4 p-2"> <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> <div className="space-y-2"> <Label htmlFor="distance" className="flex items-center"><Route className="w-4 h-4 mr-2" /> Distance</Label> <div className="flex items-center gap-2"> <Input id="distance" type="number" value={distance} onChange={e => setDistance(e.target.value)} /> <Select value={unit} onValueChange={setUnit}><SelectTrigger className="w-[80px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="km">km</SelectItem><SelectItem value="miles">mi</SelectItem></SelectContent></Select> </div> </div> <div className="space-y-2"> <Label className="flex items-center"><TimerIcon className="w-4 h-4 mr-2" /> Time</Label> <div className="grid grid-cols-3 gap-2"> <Input type="number" placeholder="hh" value={hours} onChange={e => setHours(e.target.value)} aria-label="Hours" /> <Input type="number" placeholder="mm" value={minutes} onChange={e => setMinutes(e.target.value)} aria-label="Minutes" /> <Input type="number" placeholder="ss" value={seconds} onChange={e => setSeconds(e.target.value)} aria-label="Seconds" /> </div> </div> </div> <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 text-center"> <div className="bg-cyan-500 text-white p-4 rounded-lg"><p className="text-sm opacity-80">Pace</p><p className="text-3xl font-bold tracking-tight">{paceResult.pace} <span className="text-lg">/ {unit}</span></p></div> <div className="bg-sky-500 text-white p-4 rounded-lg"><p className="text-sm opacity-80">Speed</p><p className="text-3xl font-bold tracking-tight">{paceResult.speed} <span className="text-lg">{unit}/h</span></p></div> </div> </div> ); };
const OneRepMaxCalculator = () => { const [weight, setWeight] = useState('100'); const [reps, setReps] = useState('5'); const oneRepMax = useMemo(() => { const w = parseFloat(weight); const r = parseInt(reps); if (!w || w <= 0 || !r || r <= 0) return 0; if (r === 1) return w; return w / (1.0278 - (0.0278 * r)); }, [weight, reps]); const trainingZones = [ { percentage: 95, reps: '2-3', goal: 'Max Strength' }, { percentage: 85, reps: '6-8', goal: 'Hypertrophy' }, { percentage: 75, reps: '10-12', goal: 'Endurance' }, { percentage: 60, reps: '15+', goal: 'Muscular Stamina' }, ]; return ( <div className="space-y-4 p-2"> <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> <div className="space-y-2"><Label htmlFor="lift-weight" className="flex items-center"><Dumbbell className="w-4 h-4 mr-2" /> Weight Lifted (kg)</Label><Input id="lift-weight" type="number" placeholder="e.g., 100" value={weight} onChange={e => setWeight(e.target.value)} /></div> <div className="space-y-2"><Label htmlFor="reps" className="flex items-center"><Repeat className="w-4 h-4 mr-2" /> Repetitions</Label><Input id="reps" type="number" placeholder="e.g., 5" value={reps} onChange={e => setReps(e.target.value)} /></div> </div> <p className="text-xs text-center text-muted-foreground pt-2">Formula is most accurate for reps between 2-10.</p> <div className="mt-4 text-center bg-red-500 text-white p-4 rounded-lg"><p className="text-sm opacity-80">Estimated One-Rep Max (1RM)</p><p className="text-4xl font-bold tracking-tight">{oneRepMax.toFixed(1)} kg</p></div> {oneRepMax > 0 && (<div className="pt-4"><h3 className="text-center font-semibold mb-2">Training Zones based on your 1RM</h3><div className="space-y-2">{trainingZones.map(zone => (<div key={zone.percentage} className="grid grid-cols-3 items-center p-2 bg-muted rounded-md text-sm"><div className="font-bold">{zone.percentage}%</div><div className="text-center font-semibold text-primary">{ (oneRepMax * (zone.percentage / 100)).toFixed(1) } kg</div><div className="text-right text-muted-foreground">{zone.reps} reps ({zone.goal})</div></div>))}</div></div>)} </div> ); };
...
export function HealthHub() {
    ...
    const bmr = useMemo(() => { if (!age || !weight || !height) return 0; if (gender === 'male') return (10 * weight) + (6.25 * height) - (5 * age) + 5; return (10 * weight) + (6.25 * height) - (5 * age) - 161; }, [gender, age, weight, height]);
    return (
