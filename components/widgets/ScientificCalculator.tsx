...
// Button component for our calculator grid
const CalcButton = ({ children, onClick, className }: { children: React.ReactNode; onClick: () => void; className?: string }) => (
    <Button
        variant="outline"
        className={`h-16 text-xl font-semibold rounded-lg shadow-sm hover:bg-muted/80 ${className}`}
        onClick={onClick}
    >
        {children}
    </Button>
...
export function ScientificCalculator() {
    ...
    const calculateResult = useCallback(() => {
        ...
        try {
            let evalExpression = expression.replace(/√/g, 'sqrt').replace(/%/g, '/100').replace(/π/g, 'pi');
            const calculation = evaluate(evalExpression);
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            const key = event.key;
            if (key >= '0' && key <= '9') handleButtonClick(key);
    ...
    }, [handleButtonClick, calculateResult, deleteLast, clearAll]);

    // NEW: useEffect for Dynamic Font Sizing
    useEffect(() => {
        const displayedText = result || expression || '0';
        const len = displayedText.length;

        if (len > 16) {
            setDisplayFontSize('text-2xl');
        } else if (len > 12) {
            setDisplayFontSize('text-3xl');
        } else if (len > 9) {
            setDisplayFontSize('text-4xl');
        } else {
            setDisplayFontSize('text-5xl');
        }
    }, [result, expression]);
        ...
        <Card className="w-full max-w-md mx-auto shadow-2xl shadow-gray-400/20 rounded-2xl border p-4">
            <CardContent className="p-2">
                ...
                <div className="bg-muted w-full h-28 rounded-lg mb-4 p-4 flex flex-col justify-between items-end text-right overflow-hidden">
                    ...
                    <div 
                        className={cn(
                            "font-bold h-12 break-all text-right w-full",
                            "transition-all duration-200", // Added transition for smoothness
                            displayFontSize
                        )}
                    >
                        {result || expression || '0'}
